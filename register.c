...
printf("测试用例7（用户数量达上限）执行通过 | 输入：username=\"user6\", password=\"Test123456\", confirm_password=\"Test123456\" | 预期返回-1，实际返回%d\n", ret);
    ...
    printf("测试用例11: 测试用户数达到上限\n");
    printf("预期结果: 返回-1，输出\"用户数量已达上限\"\n");
    init_test_env();
    
    // 注册足够的用户使其达到上限
    printf("正在注册 %d 个用户以填满系统...\n", MAX_USERS);
    for (int i = 0; i < MAX_USERS; i++) {
        char username[20];
        sprintf(username, "user%d", i);
        int temp_result = register_user(username, "abc123", "abc123");
        if (temp_result <= 0) {
            printf("在准备阶段注册第%d个用户失败，错误码: %d\n", i, temp_result);
            // 如果注册失败，可能是其他原因，但我们仍然继续测试
            break;
        }
    }
    
    printf("当前用户数: %d，尝试注册第 %d 个用户...\n", user_count, MAX_USERS + 1);
